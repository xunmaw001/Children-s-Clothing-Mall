...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','fenleiliebiao');
			...
			onAcrossTap(tableName){
     			uni.setStorageSync('crossTable','fenleiliebiao');
				uni.setStorageSync(`crossObj`, this.detail);
				this.$utils.jump(`../${tableName}/add-or-update?cross=true`);
			},
			// 获取详情
			async init(){
				let res = await this.$api.info('fenleiliebiao', this.id);
			...
			// 添加评论
			onCommentTap() {
				this.$utils.jump(`../discussfenleiliebiao/add-or-update?refid=${this.id}`)
			},
			// 点赞
			async zan() {
				let _this = this;
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							...
							await _this.$api.update('fenleiliebiao', _this.detail);
			...
			async onFinishTap() {
				...
				await this.$api.update('fenleiliebiao', this.detail);
				this.$utils.msg('审核成功');
